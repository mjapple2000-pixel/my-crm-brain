...
- **Missed-call / unanswered-text recovery, built into every tier.** GHL frames AI as a productivity add-on for agencies running campaigns, not a front-desk replacement for a one-truck operator, and Housecall Pro has no native AI call-answering at all. Jobber now ships an AI Receptionist that answers calls/texts and books jobs — but it's a $99/mo add-on on Grow and only bundled at no extra cost on its custom-priced Plus tier, meaning a solo operator on Jobber's entry plans still gets nothing. Our answer isn't "we're the only ones with AI" anymore — it's that AI booking is included from the first dollar, on every tier, with no add-on to upsell into.
