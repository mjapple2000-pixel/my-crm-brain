...
- **Feature bloat from the GHL/HubSpot feature-parity checklist.** We track competitor features in `features.md`, but "they have it" is not a reason to build it. Specifically:
  ...
  - **Email Marketing as a full platform** (drag-and-drop builders, A/B testing, deliverability tooling) — **Not Started, low priority.** A simple campaign/broadcast tool may eventually be worth building, but a Mailchimp-style platform is explicitly out of scope.
