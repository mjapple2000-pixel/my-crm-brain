...
---
  ...
  - **tax_rate_zip_lookup** — local cache of sales-tax rates by zip code (30-day TTL), backing the `lookup-tax-rate` edge function so quote/invoice tax lookups don't hit salestaxzip.com every time. Confirmed columns (`lookup-tax-rate/index.ts` lines 27, 76): `zip_code`, `state`, `county`, `city`, `state_rate`, `county_rate`, `city_rate`, `special_district_rate`, `combined_rate`, `source`, `imported_at`.
...
- **email_sender_rules** — per-business allow/block/keyword rules for inbound email ("EM-05"), managed from Settings → Email Config. Confirmed columns (`settings_screen.dart` lines 7984–7989): `business_id`, `rule_type` (`'block'` | `'allow'`), `match_value`, `match_scope` (`'email'` | `'domain'` | `'keyword'`), `deleted_at`. Read by `receive-email`, `gmail-inbound-webhook`, and `microsoft-graph-webhook` before a message is ever saved — allow always wins over block. Gated server-side via `check_plan_feature(business_id, "email_trust_filtering")`, plus a per-business on/off toggle at `businesses.email_trust_filtering_enabled` (default true, independent of the plan gate).
- **filtered_emails** — audit log of emails blocked by `email_sender_rules`. Confirmed columns (`receive-email/index.ts` line 603, `gmail-inbound-webhook/index.ts` line 593): `business_id`, `raw_to_address`, `raw_from_address`, `subject`, `rule_matched`, `external_message_id`. ⚠️ No screen reads this table yet — it's write-only. Flagging as an open question rather than assuming it's an oversight.
...
- **pay_periods** — a business's payroll period definitions. Confirmed columns (from `timesheets_screen.dart` line 1239 upsert and `quickbooks-sync-hours/index.ts` line 126 select): `id`, `business_id`, `week_start`, `week_end`, `locked_at`, `locked_by`, `qbo_sync_status`, `qbo_sync_attempts`. Read/written by `decide-timesheet` and `submit-timesheet`.
- **business_pay_period_config_history** — scheduled future changes to a business's pay-period settings (week start day / pay period type), so a config change can take effect on a future date without disrupting the currently-active period. Confirmed columns (`settings_screen.dart` lines 12745, 13134–13173): `business_id`, `week_start_day`, `pay_period_type`, `pay_period_config`, `effective_date`, `created_by_profile_id`, `deleted_at`. Unique on `(business_id, effective_date)` where not deleted. Read by `get-timesheets` and `get-employee-hub-data` to resolve the config in effect on a given date.
- **pay_rate_history** — the source of truth for an employee's wage history; every rate change (hourly or salary) is a new row, never an update-in-place. Confirmed columns (`employee_detail_screen.dart` line 862 insert; `get-timesheets/index.ts` line 265 select): `business_id`, `profile_id`, `pay_type`, `hourly_rate`, `annual_salary`, `effective_date`, `note`, `set_by_profile_id`, `deleted_at`. Read by `get-timesheets` to resolve the rate in effect for a given pay period.
- **pto_balances** — an employee's current PTO balance and accrual rate, separate from `pto_requests`. Confirmed columns (`pto_policy_screen.dart` line 422, `pto_requests_screen.dart` line 245): `business_id`, `profile_id`, `accrual_rate`, `balance_hours`, `updated_at`.
- **pto_balance_adjustments** — immutable audit log of every `pto_balances` change (manual edit or PTO-request approval). Confirmed columns (`pto_requests_screen.dart` line 256, `pto_policy_screen.dart` line 472): `business_id`, `profile_id`, `source` (`'manual'` | `'request_approval'`), `adjusted_by_profile_id`, `previous_balance`, `new_balance`, `delta`, `note` (nullable).
- **pay_period_audit_log** — history of actions taken on a pay period (e.g. lock/unlock). Confirmed columns from the one read site (`timesheets_screen.dart` line 1265): `id`, `pay_period_id`, `action`, `actor_user_id`, `created_at`. ⚠️ No insert call found anywhere in `lib/` or `supabase/functions/` — rows must be written by a database trigger not present in the migrations folder. Flagging for Supabase dashboard confirmation.
...
3. `snippets`, `trigger_links`, `call_logs` — resolved, RLS confirmed enabled with `business_id`-scoped policies (9/16 database check).
4. `deals` — `contact_id` FK debt resolved; now uses `lead_id` FK referencing `leads` table.
5. `employee_hub_tokens`, `job_forms`, `job_form_submissions`, `job_types`, `phone_numbers`, `routes`, `team_locations`, `time_entries`, `service_menu_items`, `stripe_connect_accounts` — resolved, all confirmed RLS-enabled with real tenant-isolation policies (9/16 database check). Note: `routes` and `team_locations` policies also require `profiles.role = 'owner'` or `profiles.permissions->>'route_management' = true`, not just matching `business_id` — a plain staff member without that permission can't see them even within their own business.
