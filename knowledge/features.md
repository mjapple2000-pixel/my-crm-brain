...
- **Issues:** `business_id` confirmed present on insert (`conversations_screen.dart` and `handle-trigger-link` edge function both write it). Full create/list/delete UI exists in the Conversations screen sidebar. RLS policy correctness still needs a Supabase dashboard check — migration files now exist in the repo but none touch this table.
